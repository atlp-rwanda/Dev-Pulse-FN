  ...
  const Columns = [
    {
      name: 'Date',
      selector: 'date',
      cell: row => <div onClick={()=>displayComment(row)}>{moment(row.date, moment.ISO_8601).format('MM/DD/YYYY HH:mm')}</div>,
      sortable: true,
    },
    {
      name: 'Session',
      cell: row => <div onClick={()=>displayComment(row)}>{getSession(row.session).name}</div>,
      sortable: true,
    },
    {
      name: 'Record',
      selector: 'attendance',
      cell: row => <div className={parseFloat(row.attendance) <1 ? 'text-red':''} onClick={()=>displayComment(row)}>{row.attendance}</div>,
      sortable: true,
    },
  ...
  return (
    <div>
      {!props.state.attendance.loading &&
        <div>
          {id && <DataTable columns={columns} data={records} customStyles={customStyles} noHeader responsive pagination />
          }
        </div>
      }
      {props.state.attendance.loading &&
