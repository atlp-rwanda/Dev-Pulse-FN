...
const ManagerView = (props) => {
    ...
    if (totalEngineers.length > 0) {
      ...
      const newData = myTrainees.map((trainee) => {
        ...
        const traineeAttendance =
          traineeAtt.length > 0
            ? traineeAtt
            : [{ trainee: trainee.id, name: trainee.name }];
      ...
      const mapData = newData.map((data) => {
        ...
        const sessionsAtt = sessionsHere.map((session) => {
          ...
          const sessionNameExists = sessions.find(
            (s) => s.id === parseInt(session)
          );
          if (sessionNameExists) {
            const sessionName = sessionNameExists.name;
            return {
              totalAverage: averageAttendance,
              [sessionName]: mean.toFixed(2),
              trainee: traineeId,
              name: myTrainees.find((t) => t.id === traineeId).name
            };
          }
          return {
            trainee: traineeId,
      ...
      const newColumns = [
        {
          ...
          cell: (row) => (
            <Link
              className='att-link'
              to={`/attendance/trainee/${row.trainee}`}
            >
              {row.name}
            </Link>
          )
        ...
        ...newSession,
        {
          ...
          cell: ({ totalAverage }) => (
            <div className={parseFloat(totalAverage) < 1 ? 'text-red' : 'text-green'}>
              {totalAverage && (
                <span>{parseFloat(totalAverage).toFixed(2)}</span>
              )}
            </div>
          ),
          sortable: true
  ...
  useEffect(() => {
    if (cohorts.cohorts && cohorts.cohorts.length > 0 && programs.programs) {
      if (cohort != 'all') {
        const ProgramByCohort = programs.programs.filter(
          (program) => program.cohortId === parseInt(cohort)
        );
  ...
  const sortByProgram = (p) => {
    if (p != 'all') {
      const trainees = cohortTrainees.filter(
        (trainee) => trainee.program === parseInt(p)
      );
  ...
  const sortByCohort = (c) => {
    if (c != 'all') {
      ...
      const trainees = allEngineers.filter(
        (trainee) => trainee.cohort === parseInt(c)
      );
        ...
        <div className='data'>
          <div className='att-controls'>
            ...
            <div className='filters'>
              <select
                className='select filter-data'
                onChange={(e) => setCohortValue(e.target.value)}
              >
                <option value='all'>All</option>
                {cohorts.cohorts &&
                  cohorts.cohorts.map(({ name, id }) => (
